...
class BookService(val bookDAO: BookDAO,private val authorDAO: AuthorDAO):BasicCrud<String,Book> {
    ...
    override fun insert(obj: Book): Book = bookDAO.insert(obj.apply { this.author = authorDAO.findById(obj.author.id.toString()).get() })//re-insert author from db to avoid inconsistency
    ...
    override fun update(obj: Book): Book{
        return if(bookDAO.existsById(obj.isbn)){//check if book exists because the save method will insert a record if does not exists
            bookDAO.save(obj.apply { this.author = authorDAO.findById(obj.author.id.toString()).get() })//re-insert author from db to avoid inconsistency
        }else{
